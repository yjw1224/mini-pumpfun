...
```text
┌────────────────────────────┐
│                     ⏱ 2h   │ ← age badge (image overlay, top-right)
│      Token image           │
│      (card width, square)  │
├────────────────────────────┤
│ Token name                 │
│ Token ticker                │
│ Market Cap                 │
│ 🔘 0x1234...abcd            │ ← creator row (avatar + address)
│ ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬░░░░░░░░░ │ ← graduation progress bar
└────────────────────────────┘
```

- Image: 카드 상단에 카드 내부 너비를 사용한 정사각형 이미지로 배치하고 object-fit cover를 적용한다.
- Age badge: 이미지 우측 상단에 반투명 검정 배경의 작은 pill badge로 Clock 아이콘 + 상대 시간 텍스트를 표시한다. 표시 규칙은 "11.3 Relative Time Formatting"을 따른다.
- Information: 이미지 아래 surface-elevated 정보 영역에 name, ticker, market cap, creator, graduation progress 순서로 표시한다.
- Hierarchy: name과 market cap은 medium emphasis, ticker는 더 작은 secondary text로 표시한다.
- Creator row: creator wallet address를 seed로 생성한 identicon 아바타(작은 원형, 16px)와 truncated address(`0x1234...abcd`)를 함께 표시한다. 아바타는 `@dicebear/identicon`으로 생성하며, 동일 주소는 항상 동일한 아바타를 갖는다.
- Graduation progress bar: creator row 바로 아래, 카드 최하단에 얇은(4px 이하) 가로 bar로 표시한다. 채워진 영역은 success(초록) color를 사용한다.
- Metadata image가 없거나 로드에 실패하면 임의 이미지 대신 동일 비율의 빈 placeholder를 표시한다.
- 가격, token address, graduated badge는 Explore Token Card에서 표시하지 않는다.
- Explore의 기본 desktop grid는 한 줄 최대 6개 카드로 구성한다. 마지막 줄의 카드 수가 6개 미만이어도 동일한 6개 grid track을 유지해 카드 크기가 커지지 않도록 한다.

Cards should not contain unnecessary badges, statistics, or decorative elements beyond the age badge, creator row, and graduation progress bar defined above.
...
## 11.3 Relative Time Formatting

Token 생성 시점부터 경과한 시간을 다음 규칙으로 축약 표시한다. 항상 내림(버림) 처리하며, 가장 큰 단위 하나만 표시한다.
...
- 1달은 30일, 1년은 365일 기준으로 근사 계산한다.
- 아이콘은 Clock(outline) 아이콘을 사용하고, 텍스트 바로 왼쪽에 배치한다.
